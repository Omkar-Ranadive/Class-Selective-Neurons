...
parser.add_argument("--mode", default='cp', type=str)
...
if args.mode == 'cp': 
    ...
    for CUR_DIR, cur_type in [(EXP_DIR, 'cs'), (RAN_DIR, 'ran')]: 
        ...
        for acc_type, layer in product(accuracy_type, channels.keys()):
            X = list(range(0, channels[layer]+1, 10)) # Stepping the channels to speed it up 
            files = []
            for f in sorted(os.listdir(CUR_DIR)): 
                if re.search('{}.*layer_{}.npy'.format(acc_type, layer), f): 
                    files.append(f)
            ...
            for file in files: 
                ...
                acc = np.load(FILE_PATH)
                acc = (acc / np.max(acc)) * 100 
                ax.plot(X, acc, label='{}'.format(file[:-4]))
                ax.set_title('Layer {}  Top:{} Accuracies Type: {}'.format(layer, acc_type, cur_type))
                # ax.legend() 
                
            fig.savefig(SAVE_DIR / '{}_{}_{}_Normalized_Accuracy_Curves_layer_{}.png'.format(datetime.now().strftime('%m_%d_%Y-%H_%M_%S'), cur_type, acc_type, layer))
            ax.clear()
